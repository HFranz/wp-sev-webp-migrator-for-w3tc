...
Unlike a runtime content filter that swaps URLs on every request, this plugin writes the replacement back into the database **once**: as soon as W3TC marks an attachment as converted, every occurrence of that image's URL in `post_content` across the whole site is rewritten from its original extension (jpg/jpeg/png/gif) to `.webp`. The attachment's own record (attached file, metadata, mime type) is updated to match, so the Media Library and REST API stay consistent too.
...
* Rewrites every reference to the converted image (full size and all intermediate sizes) in `post_content`, across all posts.
...
3. It scans `post_content` across all posts for the old URLs and rewrites them to the `.webp` versions directly in the database.
...
No. Image conversion is handled entirely by W3 Total Cache ImageService. This plugin only rewrites already-converted images' URLs, permanently, in post content and in the attachment's own record.

= How is this different from "SEV Rewrite-Free WebP for W3TC"? =

The rewrite-free plugin swaps URLs on the fly for browsers that support WebP, leaving the original content and files untouched. This plugin instead permanently rewrites the stored content and, optionally, removes the original files — useful once you no longer need to serve the original format to any visitor.
